/**
 * 국토부 아파트 실거래 일별 수집 스크립트
 * - 전월 + 당월만 수집 (당일 신고건 빠른 반영 목적)
 * - 매일 새벽 실행 → collect.ts(매월 1일 3개월치)와 병행 운영
 * - 실행: npx tsx scripts/collect-daily.ts
 */
...
function executeSQLFile(sql: string): number {
  const tmpFile = join(tmpdir(), `daily_${Date.now()}.sql`);
...
async function main() {
  console.log('=== 국토부 실거래 일별 수집 시작 (전월+당월) ===');
  ...
  async function collectWithRetry(sggCd: string, dealYmd: string): Promise<number> {
    ...
    for (let attempt = 1; attempt <= MAX_RETRY; attempt++) {
      try {
        ...
        writeLog('daily', sggCd, dealYmd, count, 'success');
        return count;
      } catch (e) {
  ...
  if (failedCombos.length > 0) {
    ...
    saveFailedLog(failedCombos, 'daily');
  }
}
